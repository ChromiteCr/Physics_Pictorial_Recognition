...
用法：
    ...
    python gen_label_gallery.py --dir data/video_frames   # 校对视频抽帧的 GLM 标注
    再用浏览器直接打开 output/label_gallery.html（或让 Claude 发布成 Artifact）。

autolabel.py / autolabel_glm.py 还在跑的时候可以随时重新执行这个脚本刷新进度。
"""
...
DEFAULT_DIR = PROJ / "data" / "self_captured"
OUT_HTML = PROJ / "output" / "label_gallery.html"

# 2026-07-14 起：spring(2)/ruler(4) 从下拉框里去掉（按要求在所有标注代码里忽略
# 这两类，但编号仍保留在 dataset.yaml 里占位，历史标注数据里的这两类框不受影响，
# 只是没有专属颜色/名称了，会用浏览器端的兜底样式显示）；base(8) 已合并进 track(1)，
# 不再单独出现。wooden_block/iron_block（6/7）是新增类，与 data/dataset.yaml、
# autolabel_glm.py 的 CLASS_NAMES 保持同步。
NAMES = {0: "cart", 1: "track", 3: "string", 5: "dynamometer",
          6: "wooden_block", 7: "iron_block"}
COLORS_HEX = {0: "#ff8000", 1: "#808080",
              3: "#c800c8", 5: "#0000ff",
              6: "#a0522d", 7: "#556b2f"}
...
def main():
    parser = argparse.ArgumentParser(description="生成标注框预览与重标网页")
    parser.add_argument("--dir", default=str(DEFAULT_DIR),
                         help="来源目录，需含 images/ 和 labels/ 子目录（默认 data/self_captured）")
